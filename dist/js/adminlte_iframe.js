...
var addTabs = function (options) {
    ...
    if (findTabPanel(pageId) === null) {
        ...
        var $title = $('<a href="javascript:void(0);" style="position: relative;"></a>').attr(pageIdField, pageId).addClass("menu_tab");
		var iconClass = $('.treeview[data-pageid="'+pageId+'"]').find('a').find('i').attr('class');
        var $icon = $("<span style='margin-right: 2px;' class='"+iconClass+"'></span>").appendTo($title);
        ...
        //是否允许关闭
        if (options.close) {
            var $i = $("<i class='fa fa-remove page_tab_close' style='cursor: pointer; position: absolute; top: 2px; right: 5px;' onclick='closeTab(this);'></i>").attr(pageIdField, pageId).appendTo($title);
...
function activeTabByPageId(pageId) {
    ...
    $(".page-tabs-content").find("i").attr('class', 'fa fa-remove')
    $(".page-tabs-content").find(".active i").attr('class', 'fa fa-power-off');
    acticeMenuli(pageId);
    scrollToTab($title[0]);
}

//激活菜单栏
function acticeMenuli(pageId){
	//先取消所有
	$('.treeview').css('background-color', '#77A6E8');
	//其次激活当前
	$('.treeview[data-pageid="'+pageId+'"]').css('background-color', '#F5FAFF').find('a').css('color', '#2D59B2');
}
    ...
    $.fn.sidebarMenu = function (options) {
        ...
        function init($menu_ul, data, level) {
            $.each(data, function (i, item) {
                ...
                //如果不是header
                var li = $('<li class="treeview " data-level="' + level + '" data-pageid="' + item.id + '"></li>');
